-- The Fallen Angel V5 (Full & Fixed - Copy cẩn thận nha ông)
...
local startButton, runButton, emoteButton, angryButton, dashButton, awakenButton, smiteButton, flyButton, dragToggle = nil, nil, nil, nil, nil, nil, nil, nil, nil
...
local function updateAnimation()
	...
	if emoteEnabled then
		if moving then
			...
			emoteButton.BackgroundColor3 = Color3.fromRGB(0, 120, 255)
...
local function activateAngryAngel()
	...
	angryActive = true
	runEnabled = false; if runButton then runButton.Text = "RUN\nOFF"; runButton.BackgroundColor3 = Color3.fromRGB(0, 170, 0) end
	stopAllTracks()
	...
	task.delay(ANGRY_DURATION, function()
		...
		startVisualCooldown(angryButton, ANGRY_COOLDOWN, "ANGRY\nANGEL", Color3.fromRGB(255, 0, 0), function() angryCooldownActive = false end)
	end)
end
...
local function activateFallenDash()
	...
	task.delay(DASH_DURATION, function()
		...
		startVisualCooldown(dashButton, DASH_COOLDOWN, "FALLEN\nDASH", Color3.fromRGB(130, 0, 255), function() dashCooldownActive = false end)
	end)
end
...
local function activateAwakening()
	...
	task.delay(AWAKEN_DURATION, function()
		...
		flying = false
		flyButton.Visible = false; flyButton.Text = "FLY\nOFF"; flyButton.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
		smiteButton.Visible = false 
		...
		startVisualCooldown(awakenButton, AWAKEN_COOLDOWN, "AWAKEN", Color3.fromRGB(255, 215, 0), function()
...
local function setupCharacter(character)
	...
	diedConn = currentHumanoid.Died:Connect(function()
		...
		if smiteButton then smiteButton.Visible = false end 
...
local function createUI()
	...
	if not uiParent then
		uiParent = player:FindFirstChild("PlayerGui") or player:WaitForChild("PlayerGui", 5)
	end
	if not uiParent then return end

	for _, child in ipairs(uiParent:GetChildren()) do
		if child.Name == "TheFallenAngelUI" then
			child:Destroy()
		end
	end
	...
	avatarImg.Image = "rbxassetid://75233801151417"
	avatarImg.Parent = mainFrame
	Instance.new("UICorner", avatarImg).CornerRadius = UDim.new(0.2, 0)

	local ytbLabel = Instance.new("TextLabel")
	ytbLabel.Size = UDim2.new(0, 70, 0, 15)
	ytbLabel.Position = UDim2.new(0, 3, 0, 68)
	ytbLabel.BackgroundTransparency = 1
	ytbLabel.Text = "ytb: KageFloppey"
	ytbLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
	ytbLabel.Font = Enum.Font.GothamSemibold
	ytbLabel.TextScaled = true
	ytbLabel.Parent = mainFrame

	local titleLabel = Instance.new("TextLabel")
	titleLabel.Size = UDim2.new(1, -90, 0, 45)
	titleLabel.Position = UDim2.new(0, 80, 0, 10) 
	titleLabel.BackgroundTransparency = 1
	titleLabel.Text = "The Fallen Angel by KageFloppey"
	...
	startButton.Position = UDim2.new(0.5, -30, 1, -45) 
	...
	runButton = createButton("RunButton", UDim2.new(1, -60, 0, 85), Color3.fromRGB(0, 170, 0), "RUN\nOFF")
	emoteButton = createButton("EmoteButton", UDim2.new(1, -60, 0, 145), Color3.fromRGB(0, 120, 255), "EMOTE\nOFF")
	angryButton = createButton("AngryButton", UDim2.new(1, -60, 0, 205), Color3.fromRGB(255, 0, 0), "ANGRY\nANGEL")
	dashButton = createButton("DashButton", UDim2.new(1, -60, 0, 265), Color3.fromRGB(130, 0, 255), "FALLEN\nDASH")
	awakenButton = createButton("AwakenButton", UDim2.new(1, -60, 0, 325), Color3.fromRGB(255, 215, 0), "AWAKEN"); awakenButton.TextColor3 = Color3.fromRGB(0,0,0)
	dragToggle = createButton("DragButton", UDim2.new(1, -60, 0, 20), Color3.fromRGB(255,140,0), "DRAG\nOFF")

	smiteButton = createButton("SmiteButton", UDim2.new(0, 10, 0, 150), Color3.fromRGB(0, 200, 255), "SMITE")
	flyButton = createButton("FlyButton", UDim2.new(0, 10, 0, 210), Color3.fromRGB(50, 50, 50), "FLY\nOFF")
	...
	for _,b in ipairs({runButton,emoteButton,angryButton,dashButton,awakenButton,smiteButton,flyButton,dragToggle}) do makeDraggable(b) end
	
	dragToggle.MouseButton1Click:Connect(function()
		dragEnabled = not dragEnabled
		dragToggle.Text = dragEnabled and "DRAG\nON" or "DRAG\nOFF"
	...
	runButton.MouseButton1Click:Connect(function()
		...
		runButton.Text = runEnabled and "RUN\nON" or "RUN\nOFF"
		...
		if not runEnabled then
			...
			startVisualCooldown(runButton, RUN_COOLDOWN, "RUN\nOFF", Color3.fromRGB(0, 170, 0), function() runCooldownActive = false end)
		end
	end)
	...
	emoteButton.MouseButton1Click:Connect(function()
		...
		emoteButton.Text = emoteEnabled and "EMOTE\nON" or "EMOTE\nOFF"
